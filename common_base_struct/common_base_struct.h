...
#define MAXSIZE_HASH_TABLE_QUERY_BATCH \
  (BATCH_SIZE * MAX_HASH_TABLE_COUNT_PER_TABLE * 2)
...
#define INVALID_REMOTEPTR \
  ((RemotePtrT){.dpuId = UINT32_MAX, .dpuAddr = UINT32_MAX})
...
static inline bool HashAddrEqual(HashAddrT a, HashAddrT b) {
  return a.edgeId == b.edgeId &&
         RemotePtrToI64(a.rPtr) == RemotePtrToI64(b.rPtr);
}

static inline void HashAddrPrint(HashAddrT hashAddr) {
  printf("(HashAddrT) {.edgeId = %d, \t.dpuId = %d, \t.dpuAddr = 0x%x}\n",
         hashAddr.edgeId, hashAddr.rPtr.dpuId, hashAddr.rPtr.dpuAddr);
}
...
static inline void MaxLinkAddrPrint(MaxLinkAddrT maxLinkAddr) {
  printf("(MaxLinkAddrT) {.dpuId = %d,\t.dpuAddr = 0x%x}\n",
         maxLinkAddr.rPtr.dpuId, maxLinkAddr.rPtr.dpuAddr);
}
...
static inline bool HashTableQueryReplyEqual(HashTableQueryReplyT a,
                                            HashTableQueryReplyT b) {
  if (a.type != b.type) {
    return false;
  }
  switch (a.type) {
...
static inline HashAddrT* MaxLinkGetHashAddrs(MaxLinkT* maxLink) {
  return (HashAddrT*)(maxLink->buffer +
                      sizeof(TupleIdT) * maxLink->tupleIDCount);
}
...
static inline int MaxLinkGetSize(int tupleIdCount, int hashAddrCount) {
  return sizeof(MaxLinkT) + tupleIdCount * sizeof(TupleIdT) +
         hashAddrCount * sizeof(HashAddrT);
}

static inline int MaxLinkBuild(int tupleIdCount, int hashAddrCount,
                               TupleIdT* tupleIds, HashAddrT* hashAddrs) {
