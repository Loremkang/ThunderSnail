...
void IOManagerSend(IOManagerT* manager) {
  ...
  DPU_ASSERT(dpu_push_xfer(*manager->dpu_set, DPU_XFER_TO_DPU, "receiveBuffer",
                           0, manager->maxSendSize, DPU_XFER_DEFAULT));
}
...
void IOManagerReceive(IOManagerT* manager) {
  ...
  DPU_ASSERT(dpu_push_xfer(*manager->dpu_set, DPU_XFER_FROM_DPU, "replyBuffer",
                           0, sizeof(DpuBufferHeader), DPU_XFER_DEFAULT));
  ...
  manager->maxReceiveSize =
      ROUND_UP_TO_8(max_in_array(NUM_DPU, manager->recvSizes));
  ...
  DPU_ASSERT(dpu_push_xfer(*manager->dpu_set, DPU_XFER_FROM_DPU, "replyBuffer",
                           0, manager->maxReceiveSize, DPU_XFER_DEFAULT));
}