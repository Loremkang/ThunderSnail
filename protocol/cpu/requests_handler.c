...
Offset* GetBufferOffsetsPtr(uint8_t* buffer) {
  ...
  return (Offset*)(buffer + totalSize -
                   ROUND_UP_TO_8(blockCnt * sizeof(Offset)));
}
...
Offset* GetBlockOffsetsPtr(uint8_t* blockPtr) {
  ...
  return (Offset*)(blockPtr + totalSize -
                   ROUND_UP_TO_8(taskCount * sizeof(Offset)));
}
  ...
  switch (resp->tupleIdOrMaxLinkAddr.type) {
    case TupleId: {
      ...
      printf("(TupleIdT){.tableId = %d, \t.tupleAddr = 0x%lx}\n",
             resp->tupleIdOrMaxLinkAddr.value.tupleId.tableId,
             resp->tupleIdOrMaxLinkAddr.value.tupleId.tupleAddr);
      break;
    }
    case MaxLinkAddr: {
      // printf("rid=%d\t", resp->taskidx);
      printf("(MaxLinkAddrT) {.dpuId = %d,\t.dpuAddr = 0x%x}\n",
             resp->tupleIdOrMaxLinkAddr.value.maxLinkAddr.rPtr.dpuId,
             resp->tupleIdOrMaxLinkAddr.value.maxLinkAddr.rPtr.dpuAddr);
      break;
    }
    case HashAddr: {
      // printf("rid=%d\t", resp->taskidx);
      printf("(HashAddrT) {.edgeId = %d, \t.dpuId = %d, \t.dpuAddr = 0x%x}\n",
             resp->tupleIdOrMaxLinkAddr.value.hashAddr.edgeId,
...
void TraverseBlock(uint8_t* blockPtr) {
  ...
  if (IsVarLenTask(taskType)) {
    ...
    OffsetsIterator taskIterator = OffsetsIteratorInit(
        VariableLengthTask, blockPtr, offsetsPtr, taskCount, 0);
    ...
    uint32_t eachTaskSize =
        (GetBlockTotalSize(blockPtr) - DPU_BUFFER_HEAD_LEN) / taskCount;
    for (int i = 0; i < taskCount; i++) {
      uint8_t* taskPtr =
          blockPtr + sizeof(BlockDescriptorBase) + eachTaskSize * i;
      ProcessTask(taskPtr, taskType);
    }
  }
}
...
void TraverseReceiveBuffer(uint8_t* buffer) {
  ...
  OffsetsIterator blockIterator = OffsetsIteratorInit(
      VariableLengthTask, buffer, offsetsPtr, GetBlockCnt(buffer), 0);
...
OffsetsIterator BlockIteratorInit(uint8_t* buffer) {
  Offset* offsetsPtr = GetBufferOffsetsPtr(buffer);
  return OffsetsIteratorInit(VariableLengthTask, buffer, offsetsPtr,
                             GetBlockCnt(buffer), 0);
}
...
OffsetsIterator TaskIteratorInit(OffsetsIterator* blockIterator) {
  ...
  if (IsVarLenTask(taskType)) {
    Offset* offsetsPtr = GetBlockOffsetsPtr(blockPtr);
    return OffsetsIteratorInit(VariableLengthTask, blockPtr, offsetsPtr,
                               taskCount, 0);
  } else {
    uint32_t eachTaskSize =
        (GetBlockTotalSize(blockPtr) - DPU_BUFFER_HEAD_LEN) / taskCount;
    return OffsetsIteratorInit(FixedLengthTask,
                               blockPtr + sizeof(BlockDescriptorBase),
                               (Offset*)NULL, taskCount, eachTaskSize);
  }
}