...
void BufferBuilderEndBlock(BufferBuilder* builder) {
  ...
  if (builder->isCurVarLenBlock) {
    ...
    __mram_ptr uint8_t* offsetsBegin =
        builder->curBlockPtr + varLenBlockDesc->blockDescBase.totalSize -
        offsetsLen;
    ...
    // flush block header
    mram_write_small(varLenBlockDesc, builder->curBlockPtr,
    ...
    // flush block header
    mram_write_small(fixedLenBlockDesc, builder->curBlockPtr,
                     sizeof(BlockDescriptorBase));
  }
  builder->curBlockPtr = replyBuffer + builder->curBlockOffset;
  // flush buffer header
  mram_write_small(&builder->bufferDesc.header, replyBuffer,
                   sizeof(DpuBufferHeader));
}
...
size_t BufferBuilderFinish(BufferBuilder* builder) {
  ...
  uint32_t paddingOffsetsLen = ROUND_UP_TO_8(offsetsLen);
  mram_write_small(builder->bufferDesc.offsets, offsetsBegin,
                   paddingOffsetsLen);
...
void BufferBuilderAppendTask(BufferBuilder* builder, Task* task) {
  switch (task->taskType) {
    case FETCH_MAX_LINK_RESP: {
      ...
      mram_write_small(task, builder->curBlockPtr + builder->curTaskOffset,
                       taskSize);
