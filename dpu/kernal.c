  ...
  for (int i = 0; i < g_decoder.bufHeader.blockCnt; i++) {
    ...
    switch (taskType) {
      ...
      case GET_OR_INSERT_REQ: {
        ...
        uint32_t slaveTaskletTaskStart =
            BLOCK_LOW(slaveTaskletId, NR_SLAVE_TASKLETS, taskCnt);
        uint32_t slaveTaskletTaskCnt =
            BLOCK_SIZE(slaveTaskletId, NR_SLAVE_TASKLETS, taskCnt);
        ...
        GetOrInsertReq* req;
        for (int j = slaveTaskletTaskStart;
             j < slaveTaskletTaskStart + slaveTaskletTaskCnt; j++) {
          ...
          // }
          primary_index_dpu* pid = IndexCheck(req->hashTableId);
          IndexGetOrInsertReq(pid, (char*)(req->ptr), req->len, req->tid,
                              req->hashTableId, &reply_buffer);
          // {
      ...
      case MERGE_MAX_LINK_REQ: {
        ...
        for (int j = slaveTaskletTaskStart;
          ...
          __mram_ptr MaxLinkEntryT* entry =
              (__mram_ptr MaxLinkEntryT*)req->ptr.dpuAddr;
      ...
      case GET_MAX_LINK_SIZE_REQ: {
        ...
        for (int j = slaveTaskletTaskStart;
          ...
          GetKthTask(&g_decoder, j, (Task*)req);
          __mram_ptr MaxLinkEntryT* entry =
              (__mram_ptr MaxLinkEntryT*)req->maxLinkAddr.rPtr.dpuAddr;
          // uint32_t size = 0;
