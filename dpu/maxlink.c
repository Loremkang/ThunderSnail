...
__mram_ptr void* new_max_link_entry_ptr =
    (__mram_ptr void*)MAX_LINK_ENTRY_START_ADDR;
...
__mram_ptr MaxLinkEntryT* NewMaxLinkEntry(MaxLinkT* ml) {
  ...
  __mram_ptr MaxLinkEntryT* maxLinkEntryPtr =
      (__mram_ptr MaxLinkEntryT*)new_max_link_entry_ptr;
...
void RetrieveMaxLinkAndAppendResp(__mram_ptr MaxLinkEntryT* entry,
                                  uint32_t taskIdx, mutex_id_t* builderMutex) {
  __dma_aligned uint8_t
      respBuf[ROUND_UP_TO_8((sizeof(FetchMaxLinkResp) + MAX_LINK_ENTRY_SIZE))];
