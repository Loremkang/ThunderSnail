...
TEST(Driver, LargeBatchSize) {
  ...
  for (int i = 0; i < TOTAL_COUNT; i += TEST_BATCH) {
    ...
    EXPECT_EQ(result, DriverBatchInsertTupleWithKeys(
                          driver, len, tupleId[1] + i, keyBuffer[1] + i));
    EXPECT_EQ(result, DriverBatchInsertTupleWithKeys(
                          driver, len, tupleId[3] + i, keyBuffer[3] + i));
    EXPECT_EQ(result, DriverBatchInsertTupleWithKeys(
                          driver, len, tupleId[5] + i, keyBuffer[5] + i));
    EXPECT_EQ(result, DriverBatchInsertTupleWithKeys(
                          driver, len, tupleId[4] + i, keyBuffer[4] + i));
    EXPECT_EQ(result, DriverBatchInsertTupleWithKeys(
                          driver, len, tupleId[2] + i, keyBuffer[2] + i));
  }
