...
TEST(NewLinkTest, DisJointSet) {
  ...
  for (int i = 0; i < MAXN; i++) {
    ...
    EXPECT_EQ(DisjointSetFind(&node[i])->hashAddrCount,
              ((i + 2) * (i + 1) / 2));
  }
}
...
TEST(NewLinkTest, NewLink) {
  ...
  newLink->maxLinkAddrCount = newLink->tupleIDCount = newLink->hashAddrCount =
      MAXN;
  for (uint32_t i = 0; i < MAXN; i++) {
    ...
    NewLinkGetHashAddrs(newLink)[i] = (HashAddrT){
        .edgeId = 1, .rPtr = (RemotePtrT){.dpuId = 12, .dpuAddr = i}};
  }
  ...
  EXPECT_EQ((uint8_t*)newLink + sizeof(NewLinkT),
            (uint8_t*)&NewLinkGetTupleIDs(newLink)[0]);
  EXPECT_EQ((uint8_t*)&NewLinkGetTupleIDs(newLink)[MAXN],
            (uint8_t*)&NewLinkGetMaxLinkAddrs(newLink)[0]);
  EXPECT_EQ((uint8_t*)&NewLinkGetMaxLinkAddrs(newLink)[MAXN],
            (uint8_t*)&NewLinkGetHashAddrs(newLink)[0]);
...
TEST(NewLinkTest, VariableLengthStructBuffer) {
  ...
  VariableLengthStructBufferT* buf =
      (VariableLengthStructBufferT*)malloc(sizeof(VariableLengthStructBufferT));
  ...
  for (int i = 1; i < MAXN; i++) {
    ...
    EXPECT_EQ(VariableLengthStructBufferAppend(buf, (uint8_t*)arr,
                                               i * sizeof(uint64_t)),
              i - 1);
  }
  free(arr);
