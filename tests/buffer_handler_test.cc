...
TEST(BufferHandler, DecodeBuffer) {
  ...
  SendGetOrInsertReq(set, 3, hashTableId, keys, tupleAddrs, TEST_BATCH,
                     recvBufs);
...
TEST(BufferHandler, BlockIteratorAndTaskIterator) {
  ...
  for (int i = 0; i < NUM_DPU; i++) {
    BufferBuilderInit(&builders[i], &bufferDescs[i], GlobalIOBuffers[i],
                      GlobalOffsetsBuffer[i], GlobalVarlenBlockOffsetBuffer[i]);
  ...
  for (int i = 0; i < TEST_BATCH; i++) {
    ...
    switch (responses[i].tupleIdOrMaxLinkAddr.type) {
      ...
      case HashAddr:
        ...
        EXPECT_EQ(responses[other].tupleIdOrMaxLinkAddr.value.tupleId.tableId,
                  1);
        EXPECT_EQ(responses[other].tupleIdOrMaxLinkAddr.value.tupleId.tupleAddr,
                  i);
        break;
      default:
        FAIL();
