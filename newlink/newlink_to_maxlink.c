...
void BuildPreMaxLinkFromNewLink(VariableLengthStructBufferT* newLinkBuffer,
  ...
  for (OffsetT i = 0; i < newLinkBuffer->count; i++) {
    ...
    int size = NewLinkGetSize(merger.tupleIDCount, merger.maxLinkAddrCount,
                              merger.hashAddrCount);
    int idx =
        VariableLengthStructBufferAppendPlaceholder(preMaxLinkBuffer, size);
    PreMaxLinkT* preMaxLink =
        (PreMaxLinkT*)VariableLengthStructBufferGet(preMaxLinkBuffer, idx);
    Unimplemented("Fix Error");
  ...
  for (OffsetT i = 0; i < preMaxLinkBuffer->count; i++) {
    PreMaxLinkT* preMaxLink =
        (PreMaxLinkT*)VariableLengthStructBufferGet(preMaxLinkBuffer, i);
