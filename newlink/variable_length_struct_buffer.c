...
static inline bool ExpendableBufferExpand(ExpendableBufferT* buf,
                                          OffsetT capacity) {
...
OffsetT ExpendableBufferAppend(ExpendableBufferT* buf, uint8_t* data,
                               OffsetT length) {
...
OffsetT VariableLengthStructBufferAppendPlaceholder(
  ...
  OffsetT count =
      ExpendableBufferAppend(buf->offset, (uint8_t*)&offset, sizeof(OffsetT)) /
      sizeof(OffsetT);
  ValidValueCheck(count == buf->count);
  return buf->count++;
}

OffsetT VariableLengthStructBufferAppend(VariableLengthStructBufferT* buf,
