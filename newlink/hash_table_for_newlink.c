...
void HashTableForNewLinkSoftReset(HashTableForNewLinkT* hashTable) {
  ...
  if (hashTable->capacity > 0) {
    memset(hashTable->entries, 0,
           sizeof(HashTableForNewLinkEntryT) * hashTable->capacity);
  }
}

// automatically reset while expand
void HashTableForNewLinkExpandAndSoftReset(HashTableForNewLinkT* hashTable,
                                           int capacity) {
...
static inline int TryGetIdAtPosition(HashTableForNewLinkT* hashTable,
                                     int slotId, HashTableQueryReplyT key) {
  if (hashTable->entries[slotId].value == 0) {  // empty slot. insert
    hashTable->entries[slotId].key = key;
    hashTable->entries[slotId].value =
        ++hashTable->count;  // id starting from 1
    return hashTable->entries[slotId].value;
  } else if (HashTableQueryReplyEqual(hashTable->entries[slotId].key,
                                      key)) {  // match
    return hashTable->entries[slotId].value;
  } else {  // mismatch
    return 0;
...
static inline uint64_t Hash(HashTableQueryReplyT key) {
  ...
  switch (key.type) {
    case TupleId:
      hash_result ^= hash64_2((uint64_t)key.value.tupleId.tableId) ^
                     hash64_2((uint64_t)key.value.tupleId.tupleAddr);
      break;
    case MaxLinkAddr:
...
int HashTableForNewlinkGetId(HashTableForNewLinkT* hashTable,
  ...
  for (int slotId = NextSlot(hashTable, firstSlotId); slotId != firstSlotId;
       slotId = NextSlot(hashTable, slotId)) {
