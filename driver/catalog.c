...
void CatalogInitTable(CatalogT* catalog, TableIDT tableId, size_t edgeCount, EdgeIDT *edgeIDs) {
    ...
    catalog->tableCount++;
    OffsetT offset = VariableLengthStructBufferAppendPlaceholder(&catalog->edges, edgeCount * sizeof(EdgeIDT));
...
EdgeIDT CatalogEdgeIdGet(CatalogT* catalog, TableIDT tableId, OffsetT hashTableIndex) {
    ArrayOverflowCheck(hashTableIndex < CatalogHashTableCountGet(catalog, tableId));
