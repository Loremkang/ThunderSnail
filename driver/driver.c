...
void DriverInit(DriverT* driver) {
  ...
  IOManagerInit(&driver->ioManager, &driver->dpu_set, GlobalIOBuffers,
                GlobalOffsetsBuffer, GlobalVarlenBlockOffsetBuffer,
                GlobalIOBuffers);
...
size_t RunGetOrInsertWithKeys(CatalogT* catalog, IOManagerT* ioManager,
                              struct dpu_set_t* set, int batchSize, int tableId,
                              TupleIdT* tupleIds, TupleIdT* resultTupleIds,
                              HashTableQueryReplyT* resultCounterpart,
                              KeyT* keys) {
  printf("===== %s =====\n", __func__);
  int hashTableCount = CatalogHashTableCountGet(catalog, tableId);
...
void PrintGetOrInsertResult(CatalogT* catalog, int batchSize, int resultSize,
                            int tableId, TupleIdT* resultTupleIds,
                            HashTableQueryReplyT* resultCounterpart) {
  printf("===== %s =====\n", __func__);
  int hashTableCount = CatalogHashTableCountGet(catalog, tableId);
  int resultCount = 0;
...
void GetMaximumMaxLinkInNewLink(IOManagerT* ioManager,
  ...
  for (OffsetT i = 0; i < newLinkBuffer->count; i++) {
    ...
    if (newLink->maxLinkAddrCount > 0) {
      ValidValueCheck(largestMaxLinkPos[i] >= 0 &&
                      largestMaxLinkPos[i] < newLink->maxLinkAddrCount);
    ...
    for (OffsetT i = 0; i < newLinkBuffer->count; i++) {
      ...
      for (int j = 0; j < newLink->maxLinkAddrCount; j++) {
        if (j == largestMaxLinkPos[i]) {
          ...
          uint8_t* task =
              OffsetsIteratorGetKthData(&taskIterators[dpuId], taskIdxInBlock);
          FetchMaxLinkResp* resp = (FetchMaxLinkResp*)task;
          // MaxLinkAddrPrint(addr);
      ...
      Offset offset =
          VariableLengthStructBufferAppendPlaceholder(preMaxLinkBuffer, size);
      NewLinkT* preMaxLink =
          (NewLinkT*)VariableLengthStructBufferGet(preMaxLinkBuffer, offset);
...
void InsertPreMaxLink(IOManagerT* ioManager,
  ...
  ArrayOverflowCheck(preMaxLinkBuffer->count <= MAXSIZE_HASH_TABLE_QUERY_BATCH);
  printf("===== %s =====\n", __func__);
  ...
  int newMaxLinkTaskCount = 0;
  {
    ...
    for (OffsetT i = 0; i < preMaxLinkBuffer->count; i++) {
      ...
      if (preMaxLink->maxLinkAddrCount == 0) {
        ...
        size_t size =
            ROUND_UP_TO_8(sizeof(NewMaxLinkReq) +
                          preMaxLink->hashAddrCount * sizeof(HashAddrT) +
                          preMaxLink->tupleIDCount * sizeof(TupleIdT));
        uint8_t* task =
            IOManagerAppendPlaceHolder(ioManager, dpuId, taskType, size);
  ...
  for (int dpuId = 0; dpuId < NUM_DPU; dpuId++) {
    ...
    for (; OffsetsIteratorHasNext(&blockIterator);
      ...
      for (; OffsetsIteratorHasNext(&taskIterator);
        ...
        NewLinkT* preMaxLink =
            (NewLinkT*)VariableLengthStructBufferGet(preMaxLinkBuffer, taskIdx);
...
void PrintNewMaxLinkAddrs(VariableLengthStructBufferT* preMaxLinkBuffer,
...
uint32_t UpdateHashTableAndGetResult(
    IOManagerT* ioManager, VariableLengthStructBufferT* preMaxLinkBuffer,
...
uint32_t DriverBatchInsertTupleWithKeys(DriverT* driver, int batchSize,
  ...
  size_t resultCount = RunGetOrInsertWithKeys(
      &driver->catalog, &driver->ioManager, &driver->dpu_set, batchSize,
      tableId, tupleIds, driver->resultTupleIds, driver->resultCounterpart,
      keys);
  ...
  driver->totalValidMaxLinkCount = UpdateHashTableAndGetResult(
      &driver->ioManager, &driver->preMaxLinkBuffer, driver->newMaxLinkAddrs,
      driver->validMaxLinkCount);
